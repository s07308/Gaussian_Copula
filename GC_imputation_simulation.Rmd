...
$y_i > y_{i'}, \delta_{i'} = 1$
```{r}
df.1 <- df.pair[df.pair[, 2] > df.pair[, 6] & df.pair[, 7] == 1, ]
b.imputed.1 <- rep(1, nrow(df.1))
```
...
$y_i < y_{i'}, \delta_i = 1$
```{r}
df.4 <- df.pair[df.pair[, 2] < df.pair[, 6] & df.pair[, 3] == 1, ]
b.imputed.4 <- rep(0, nrow(df.4))
```

**Case 5** <br>
$y_i < y_{i'}, \delta_i = 0, \delta_{i'} = 1$
```{r}
...
```{r}
nrow(df.pair[df.pair[, 4] > df.pair[, 8], ])
sum(c(b.imputed.1, b.imputed.2, b.imputed.3, b.imputed.4, b.imputed.5, b.imputed.6))
```

**Case 7** <br>
$y_i > y_{i'}, \delta_{i'} = 1$
```{r}
df.7 <- df.pair[df.pair[, 2] > df.pair[, 6] & df.pair[, 7] == 1, ]
c.imputed.7 <- rep(0, nrow(df.7))
```
...
$y_i < y_{i'}, \delta_i = 1$
```{r}
df.10 <- df.pair[df.pair[, 2] < df.pair[, 6] & df.pair[, 3] == 1, ]
c.imputed.10 <- rep(1, nrow(df.10))
```
...
```{r}
nrow(df.pair[df.pair[, 4] < df.pair[, 8], ])
sum(c(c.imputed.7, c.imputed.8, c.imputed.9, c.imputed.10, c.imputed.11, c.imputed.12))
```
